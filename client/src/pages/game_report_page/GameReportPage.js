...
export default class GameReport extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("/api/v1/get-video-status?date=" + date.toISOString().split("T")[0])
      .then((res) => {
        console.log("In the get video status " + res.data);
  ...
  updateGame = (game_id) => {
    if (game_id) {
      ...
      axios.get("/api/v1/get-score?game_id=" + game_id).then((res) => {
        ...
        axios
          .get(
            "/api/v1/get-team-report-daily?team_id=" +
              response[0].home_team_id +
              "&date=" +
              date
          )
          .then((res) => {
  ...
  render() {
        ...
        <div class="score-container">
          <div class="team-wrapper">
            <div class="team-details">
              ...
              <div class="record">
                <h6>
                  {this.state.score[0].away_team_wins} -{" "}
                  {this.state.score[0].away_team_losses}
                </h6>
              </div>
            </div>
            <div class="team-score">
              <h2
                style={{
                  ...(this.state.score[0].away_team_score >
                  this.state.score[0].home_team_score
                    ? { "font-weight": "bold" }
                    : {}),
                }}
              >
          ...
          <div class="game-status">{this.state.score[0].status}</div>
          <div class="team-wrapper">
            <div class="team-score">
              <h2
                style={{
            ...
            <div class="team-details">
              ...
              <div class="record">
                <h6>
                  {this.state.score[0].home_team_wins} -{" "}
                  {this.state.score[0].home_team_losses}
                </h6>
              </div>
            </div>
          </div>
    ...
    if (
      this.state.gotGameReport &&
      this.state.gotScore &&
      this.state.gotHomeTeamReportDaily &&
      ...
      tabs = (
        <Tabs
          awayTeamName={this.state.score[0].away_team_name}
          homeTeamName={this.state.score[0].home_team_name}
          homeTeamReport={homeTeamReport}
          awayTeamReport={awayTeamReport}
          homeTeamReportDaily={this.state.homeTeamReportDaily}
          awayTeamReportDaily={this.state.awayTeamReportDaily}
    ...
    return (
      <div class="main-container">
        <div class="game-filters-container filters-container">
          ...
          <div class="game-selector">
            <Search
              data={this.state.video_status}
              updateFunction={this.updateGame}
              updateVariable={this.state.gameUpdateVariable}
              optionFunc={this.getGameOption}
        ...
        <div class="game-report-container report-container">
          ...
          <br />
          <div class="accordion-game-report">{tabs}</div>
        </div>
      </div>
    );
  }
